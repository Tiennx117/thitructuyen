...
class LoadingUI extends Component {
  ...
  render() {
    ...
    const classes = loading ? `loadingoverlay-ui ${className}` : className;
    const renderChilds = !loading || renderChildren;

    return (
      <Tag {...attributes} className={classes} aria-busy={loading}>
        {loading &&
        <div tabIndex="0" onKeyUp={this.tabbedUpTop} onKeyDown={this.tabbedDownTop} ref={this.setTopFocus} />}
        {renderChilds && children}
        {loading &&
        <div className="loadingoverlay-ui-container"
          tabIndex="0"
          ref={this.setBlocker}
          onKeyUp={this.tabbedUpBottom}
          onKeyDown={this.tabbedDownBottom}
        >
          <div className="loadingoverlay-ui-overlay" ref={this.setContainer} />
          <div className="loadingoverlay-ui-message-container"
            ref={this.setMessageContainer}
            style={{ top: keepInView ? this.state.top : undefined }}
          >
            <div className="loadingoverlay-ui-message">
              {message}
...
LoadingUI.defaultProps = defaultProps;
