...
function ViewAllLearner(props) {
    ...
    const filterCourse = async (value) => {
        setLoading(true);
        let result = await learnerService.getviewallmylearnings(value);
                        ...
                        <Card style={{ boxShadow: 'none',height: 'calc(100vh - 300px)' }} title=
                            {
                                <div className='d-flex flex-row justify-content-between header-fix' >
                                    ...
                                    <span className='d-flex flex-column align-self-center justify-content-end mr-3' >
                                        <Link to={'/learner/my-learning?isBack=1'} style={{ textDecoration: 'underline', color: '#B7A9A9', fontWeight: 500 }} className='fsx-10px text-uppercase'> Quay lại </Link>
                                    </span>
                                </div>
                            }
