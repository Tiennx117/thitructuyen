...
const LearningDetailContainer = (props) => {
    ...
    const [courseLauchID] = useState(props.idCourse);
    ...
    useEffect(() => {
        ...
        if (props.idCourse && visibleDialog1 == false) {
            loadApi();
        }
    }, [props.idCourse, visibleDialog1]);
    ...
    const loadApi = async () => {
        ...
        setLoading(true);
        //tong quan
        switch (props.nameCourseType) {
            case "T":
                //Lớp học tập trung
                let resultTT = await learningService.getassignedclassroomtrainingdetail(props.idCourse);
                ...
                break;
            default:
                // let result = await learningService.getmylearningdetail(props.idCourse, props.isMyLearning);
                let result = await learningService.getmylearningdetailnew(props.idCourse, props.isMyLearning);
                ...
                // let result1 = await learningService.getmylearningnodedetail(props.idCourse, props.isMyLearning, props.isNodeClick);
                let result1 = await learningService.getmylearningnodedetailnew(props.idCourse, props.isMyLearning, props.isNodeClick);
                ...
                //Set viewTypeProgram = true
                if (result.data.LearningItem?.LearningFlag != "" && result1.data.LearningNodeItems.some(item => item.ParentNodeID > 0)) {
                    setViewTypeProgram(true);
                }
    ...
    function setWithTabView() {
        ...
        let withTab = props.nameCourseType == "T" ? screenWidth <= 767 ? '100%' : '377px' : screenWidth <= 767 ? '100%' : '483px';
        return withTab;
    }
    useEffect(() => {
        setWithTabView();
    ...
    function toExamDetail(node) {
        ...
        if (node.NodeTypeName.toLowerCase() == "xapi tincan") {
            ...
            var timer = setInterval(function () {
                if (myWindow3.closed) {
                    clearInterval(timer);
                    trackCourseTinCan(node, isoString);
                    //loadApi();
                }
            }, 1000);
            return;
    ...
    const trackCourseTinCan = async (node, isoString) => {
        const params = {
            "NodeID": node.NodeID,
            "ParentNodeID": node.ParentNodeID,
            // "UserID": 41215,
        ...
        let result = await learningService.trackcoursetincan(params, isoString);
        if (result.data.Completion != null) {
            await learningService.updateNodeTinCan(params, result.data)
        }
        loadApi();
    }

    const openVideoDisplay = async (node) => {
        if (node.MaxAttemptAllow.split('/')[0] == "0") {
    ...
    console.log('CourseTypeName', dataCourse)
    return (
        <div>
            <LoadingPanel loading={loading}>
                ...
                <div className='d-flex flex-column flex-3 pr-3 pl-3'>
                        ...
                        <div className='w-100 custom-detail-learning' style={{ position: "relative", marginTop: '-13px' }}>
                            ...
                            <TabView style={{ top: '-13px', width: setWithTabView(), margin: '0 auto', overflow: 'auto', backgroundColor: 'white', border: '1px solid #ccc', borderRadius: '6px' }} activeIndex={activeIndex1} onTabChange={(e) => setActiveIndex1(e.index)}>
                                <TabPanel header="Tổng quan ">
                                    {props.nameCourseType == "T" ?
                                        <div>
                                            ...
                                            <div >
                                                <div className='p-3 list-item' style={{ marginTop: '3rem' }}>
                                                    <span>
                                                        {expand == true ?
                                        ...
                                        <div>
                                            <div className='row mb-3' style={{ height: '115px' }}>
                                                ...
                                                >
                                                    <div className="top-start d-flex flex-row justify-content-between icon-learning p-4">
                                                        <div className=''>
                                                            <p>
                                                                <span className="date-head"><i className="fas fa-calendar-days"></i> Ngày bắt đầu: </span>
                                            ...
                                            {viewTypeProgram == true ?
                                                <div className='d-flex flex-row'>
                                                    <div className='col-3 list-folder-left' style={{ marginTop: '3rem' }}>
                                                    ...
                                                    <div className='p-3 list-item list-node-right' style={{ marginTop: '3rem' }}>
                                                        <div >
                                                                                            ...
                                                                                            <>
                                                                                                ...
                                                                                                {item.children.map((childItem, idx) => (
                                                                                                    <React.Fragment key={idx}>
                                                                                                        <LearningDetailNodeItem isMyLearningAPINode={isMyLearningAPINode} dataNodeTree={childItem} idCourse={props.idCourse} courseCode={CourseCode} nodeName={nodeName} dataLopHocTT={dataLopHocTT} dataCourse={dataCourse} loadApiFromNodeItem={() => loadApi()} clickPlay={(node) => toExamDetail(node)} modeViewType={viewTypeProgram} courseType={courseType} />
                                                                                                    </React.Fragment>
                                                                                                ))}
                                                                                            </>
                                                                                            :
                                ...
                                <TabPanel header="Đánh giá xếp hạng ">
                                    <RatingContainer courseType={props.nameCourseType} courseLauchID={courseLauchID} isRated={isRated} iLTStatus={iLTStatus} />
                    ...
                    <Sidebar className='sidebar-header-none' visible={visibleRightFB} onHide={() => closeRightFB()} position='right' style={{ width: '70%' }}>
                        {
                            <>
                                ...
                                <FormFeedBackContainer iltID={props.idCourse} onShow={displayBasic} ILTName={dataLopHocTT.Name} sessionID={sessionId} formID={formId} onHiden={() => onHidenFormFeedback()} /></>
                    ...
                    <Sidebar className='sidebar-header-none' visible={visibleDetail} onHide={() => closeDetail()} position='right' style={{ width: '70%' }}>
                        {
                            <>
                                ...
                                <FormDetailFeedBack iltID={props.idCourse} ILTName={dataLopHocTT.Name} sessionID={sessionId} dataDetail={dataDetail} formID={formId} /></>
                        }
                    </Sidebar>
...
LearningDetailContainer.propTypes = {
...
export default LearningDetailContainer