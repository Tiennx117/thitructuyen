...
const orderService = {
    ...
    filterPage(advanceSearch){
        ...
        if(advanceSearch.from_date) url += '&from_date=' + advanceSearch.from_date;
        if(advanceSearch.end_date) url += '&end_date=' + advanceSearch.end_date;
        if(advanceSearch.state) url += '&state=' + advanceSearch.state;
    ...
    changeState(ids, state){
        return axios.post(path + '/changestate?state='+state, ids);
    }
